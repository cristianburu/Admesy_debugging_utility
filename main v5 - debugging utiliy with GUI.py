...
from spectrum_colormap_calculator import generateGradient
...
class MainUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.pushButton_hera_find_int_time.clicked.connect (self.hera_find_int_time)

        self.lineEdit_get_requested_sat.setValidator (QtGui.QDoubleValidator (0.03, 0.97, 2, notation=QtGui.QDoubleValidator.StandardNotation))
    ...
    def handle_focus_changed(self, old_widget, now_widget):
        ...
        if self.lineEdit_get_requested_sat is old_widget:
            try:
                get_requested_sat = float (self.lineEdit_get_requested_sat.text())
                if (get_requested_sat > 0.97) or (get_requested_sat < 0.03):
                    self.lineEdit_get_requested_sat.clear()
            except ValueError:
                self.lineEdit_get_requested_sat.clear()
    ...
    def disable_interface_hera (self):
        ...
        self.lineEdit_spectrum_clip.clear()
        self.label_save_file.clear()
        self.lineEdit_get_requested_sat.clear()
        self.lineEdit_int_time_result.clear()
        ...
        # Disable checkboxes
        self.checkBox_show_spectrum.setChecked(False)
        self.checkBox_save_to_file.setChecked(False)
        self.checkBox_auto_set_int_time.setChecked(False)
    ...
    def enable_interface_hera (self):
        ...
        # Enable checkboxes
        self.checkBox_auto_set_int_time.setChecked(True)
    ...
    def hera_measure_spectrum_fast (self):
    ...
    def hera_measure_spectrum (self):
        # Close spectrum window in case is open
        plt.close("all")
        ...
        # Do we want to remove negative values? I think so...
        spectrum = numpy.clip(spectrum,a_min=0,a_max=None)

        if self.checkBox_save_to_file.isChecked():
            saved_file_string = "Spectrum " + str(ctime()[11:13]) + "_" + str(ctime()[14:16]) + "_" + str(ctime()[17:19]) + ".csv"
            print (saved_file_string)
            savetxt(saved_file_string, [spectrum], delimiter=",", fmt="%.6f")
            self.label_save_file.setText (f"Saved in file:  {saved_file_string}")

        self.lineEdit_spectrum_clip.setText(f"{clip_value:0.3f}")
        ...
        if self.checkBox_show_spectrum.isChecked():
            x_axis = self.hera_get_wavelengths()

            print ("X_AXIS")
            print ("----------------------------------")
            print (x_axis)

            plt.xticks (numpy.arange(x_axis[0], x_axis[-1]+50, step=50, dtype='int'))
            plt.plot(x_axis,spectrum,color="k",linewidth=0.5)

            font_for_plot = {'family': "sans-serif",
                            'color':  'black',
                            'weight': 'normal'
                            }
            plt.xlabel("Wavelength (nm)",fontdict=font_for_plot)
            plt.ylabel("W/m\u00b2 x sr x nm",fontdict=font_for_plot)

            colormap_visible = True
            if colormap_visible:
                # Retrieve polygon to be plotted
                polygon = plt.fill_between(x_axis, spectrum, 0, lw=0, color='none')

                # Retain limits because imshow is setting them tight
                xlim = plt.xlim()
                ylim = plt.ylim()
                ...
                nr_of_colors = 256
                interp_str = "bicubic"
                first_wl = int (x_axis[0])
                last_wl = int (x_axis[-1])

                # Generate colormap
                visibleSpec = mcolors.LinearSegmentedColormap("VisibleSpectrum", generateGradient(nr_of_colors,first_wl,last_wl), N=nr_of_colors)
                print (f"Colormap has length of {visibleSpec.N}")

                # Render fill between image
                resolution_of_gradient = 256
                verts = numpy.vstack([p.vertices for p in polygon.get_paths()])
                gradient = plt.imshow(numpy.linspace(0, 1, resolution_of_gradient).reshape(1, -1), cmap=visibleSpec, interpolation=interp_str,aspect='auto',
            ...
            plt.get_current_fig_manager().set_window_title('Spectrum Plot')
            plt.tight_layout()
            plt.show()
    ...
    def hera_get_wavelengths (self):
        ...
        if (step_wl == 0.5) or (step_wl == 2.5):
            wl_info = f"{int(first_wl)} nm - {int (last_wl)} nm  /  Resolution:{step_wl:0.1f} nm"
        else:
            wl_info = f"{int(first_wl)} nm - {int (last_wl)} nm  /  Resolution:{int (step_wl)} nm"

        self.lineEdit_get_wl.setText (wl_info)

        # savetxt("x_axis.csv", [wl], delimiter=",", fmt="%.1f")


        print (first_wl, last_wl, step_wl)
        return wl

    def hera_find_int_time (self):

        # Check if requested saturation is valid
        try:
            sat_requested = float (self.lineEdit_get_requested_sat.text())
            if (sat_requested>0.97) or (sat_requested<0.03):
                self.statusbar.showMessage (f"Invalid value for sensor saturation!", 10000)
                return
        except ValueError:
            self.statusbar.showMessage (f"Invalid value for sensor saturation!", 10000)
            return

        # Get autorange / int. time / average to reapply at end
        ...
        print ("Values read before starting -FIND INT TIME-")
        print (f"Autorange is {ar_read}")
        print (f"Int. time is {int_time_read}")
        print (f"Avearge is {avg_read}")


        # Set Auto-range = OFF, Int. time = 50000, Avg = 1 - Probably we need more robust error handling

        # Write to device autorange - Internal memory
        command_py = ":SENSe:SP:AUTORANGE 0\n"
        buffer_length = len (command_py)
        timeout_ms = 5000
        error_write = py_usbtmc_write(ptr_handle_spectro, command_py.encode('ASCII'), buffer_length, timeout_ms)
        ...
        # Write to device avg - Internal memory
        command_py = ":SENSe:SP:AVERAGE 1\n"
        buffer_length = len (command_py)
        timeout_ms = 5000
        error_write = py_usbtmc_write(ptr_handle_spectro, command_py.encode('ASCII'), buffer_length, timeout_ms)

        # Set initial time at 2500
        working_time = 2500
        sensor_sat = 0
        
        while sensor_sat < sat_requested:
            # Write to device int time - memory
            command_py = ":SENSe:INT " + str (working_time) +"\n"
            buffer_length = len (command_py)
            timeout_ms = 5000
            error_write = py_usbtmc_write(ptr_handle_spectro, command_py.encode('ASCII'), buffer_length, timeout_ms)
            ...
            sensor_sat = self.hera_measure_spectrum_fast()
            print (f"Sensor sat for {working_time} μs is {sensor_sat:0.3f}")

            working_time = 2 * working_time

        max_time = working_time/2
        min_time = working_time/4
        sensor_sat = 0
        print (f"Max time is {max_time}")
        print (f"Min time is {min_time}")
        print (f"Threshold is {(abs(sensor_sat-sat_requested)*100/sat_requested)}")

        iteration = 0

        while ((abs(sensor_sat-sat_requested)*100/sat_requested) > 0.1) and (iteration < 50):
            iteration = iteration + 1
            working_time = int ((min_time+max_time)/2)
            ...
            sensor_sat = self.hera_measure_spectrum_fast()
            print(f"Sensor sat for {working_time} μs is {sensor_sat:0.3f}. Threshold is {(abs(sensor_sat-sat_requested)*100/sat_requested):0.2f}%")
            print("---------------------------------------------------")
            

            if sensor_sat < sat_requested:
                min_time=working_time
            else:
                max_time=working_time

        self.lineEdit_int_time_result.setText(f"{working_time}")


        if self.checkBox_auto_set_int_time.isChecked():
            # Reapply only avg
            
            # Write to device avg - Internal memory
            command_py = ":SENSe:SP:AVERAGE " + str (avg_read) + "\n"
            buffer_length = len (command_py)
            timeout_ms = 5000
            error_write = py_usbtmc_write(ptr_handle_spectro, command_py.encode('ASCII'), buffer_length, timeout_ms)
        else:
            # Reapply old Auto-range / Int. time / Avg 

            # Write to device autorange - Internal memory
            command_py = ":SENSe:SP:AUTORANGE " + str(ar_read) + "\n"
            buffer_length = len (command_py)
            timeout_ms = 5000
            error_write = py_usbtmc_write(ptr_handle_spectro, command_py.encode('ASCII'), buffer_length, timeout_ms)

            # Write to device int time - memory
            command_py = ":SENSe:INT " + str (int_time_read) +"\n"
            buffer_length = len (command_py)
            timeout_ms = 5000
            error_write = py_usbtmc_write(ptr_handle_spectro, command_py.encode('ASCII'), buffer_length, timeout_ms)
